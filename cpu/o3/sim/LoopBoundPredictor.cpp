 ...
 * @return false 
 */
bool LoopBoundaryPredictor::UpdateLoopBoundaryDetector(uint64_t stride_load_pc,uint64_t branchPC,int64_t imm, uint8_t rs1,uint64_t src1, uint8_t rs2, uint64_t src2) {
    // 每一次找到分支都会进行查找
    if (imm >= 0 || stride_load_pc < branchPC + imm) return false;  // 非向后跳转)
    //更新lc
    ...
    if (!hit) { // 如果没有命中
        LoopBundaryPredictorComponents lbp(tag,(LastCompare){branchPC,rs1,src1,rs2,src2},ewma_counter_max);
        lbps[idx] = lbp;
        return false;
    }
    else { // 表项存在
        LoopBundaryPredictorComponents  &lbp = it->second;
        if (!lbp.ComparePC(branchPC)) { // TODO: 如果pc不符合 ,直接换,不考虑置信度会出什么问题嘛
            ...
            lbp.UpdateLoopIncrement(src1,src2);
            return true;
        }
    }
    return false;
 ...
 * @return uint32_t 
 */
void LoopBoundaryPredictor::LoopBoundaryDetector::UpdateLoopIncrement(uint64_t src1,uint64_t src2) {
    if (this->lc.src1 == src1 && this->lc.src2 != src2) {
        this->loop_increment = (int64_t)(src2 -  this->lc.src2);  // 步进值 , 预取值小于循环边界，最后一次时
        this->loop_number = (uint64_t)((int64_t) (lc.src1 - lc.src2)/this->loop_increment);
        this->lc.src2 = src2;  // 不更新的话步进值会越来越大,但是更新的话，预取值会越來越小的
    }
    else if (this->lc.src2 == src2 && this->lc.src1 != src1) {
        this->loop_increment = (int64_t) (src1 - this->lc.src1);  // 步进值
        this->loop_number = (uint64_t)((int64_t)(lc.src2 - lc.src1)/this->loop_increment);
        this->lc.src1 = src1;
    }
    else if (this->lc.src2 != src2 && this->lc.src2 != src2) {
        this->lc.src1 = src1;
        this->lc.src2 = src2;
        int64_t temp = (int64_t)(lc.src2 - lc.src1)/this->loop_increment;
        this->loop_number =  temp < 0 ? -temp : temp;
    }
    // std::cout << "number " << loop_number  << " increment " << loop_increment << std::endl;
}
